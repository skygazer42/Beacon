...
namespace {
    ...
    double beacon_env_double(const char *name, double def_value) {
        const char *raw = ::getenv(name);
        if (!raw) {
            return def_value;
        }
        try {
            return ::strtod(raw, nullptr);
        } catch (...) {
            return def_value;
        }
    }

    std::string beacon_env_str(const char *name) {
        const char *raw = ::getenv(name);
        if (!raw) {
            return "";
        }
    ...
    std::string beacon_to_lower_hex(std::string s) {
        for (auto &c : s) {
            if (c >= 'A' && c <= 'F') {
                c = (char)('a' + (c - 'A'));
            }
    ...
    std::string beacon_hex_no_spaces(const void *buf, size_t len) {
    ...
    std::string beacon_random_hex(size_t bytes) {
        const std::string raw = makeRandStr((int)bytes, false);
        return beacon_hex_no_spaces(raw.data(), raw.size());
    }

    bool beacon_parse_traceparent(const std::string &traceparent, std::string &trace_id, std::string &parent_span_id, bool &sampled) {
        trace_id.clear();
        parent_span_id.clear();
        sampled = false;

        std::string tp = std::string(traceparent);
        trim(tp);
        if (tp.empty()) {
            return false;
        }
        ...
        std::string version = beacon_to_lower_hex(trim(std::move(parts[0])));
        std::string tid = beacon_to_lower_hex(trim(std::move(parts[1])));
        std::string sid = beacon_to_lower_hex(trim(std::move(parts[2])));
        std::string flags = beacon_to_lower_hex(trim(std::move(parts[3])));

        if (version.size() != 2 || tid.size() != 32 || sid.size() != 16 || flags.size() != 2) {
            return false;
        }
        if (!beacon_is_lower_hex(version) || !beacon_is_lower_hex(tid) || !beacon_is_lower_hex(sid) || !beacon_is_lower_hex(flags)) {
            return false;
        }
    ...
    std::string beacon_normalize_zipkin_endpoint(const std::string &raw) {
        std::string s(raw);
        trim(s);
        if (s.empty()) {
            return "";
        }
        // Accept full endpoint: http://host:9411/api/v2/spans
        if (end_with(s, "/api/v2/spans")) {
            return s;
        }
        // Accept base: http://host:9411
        if (end_with(s, "/")) {
    ...
    std::string beacon_derive_zipkin_from_otlp(const std::string &otlp_endpoint) {
        std::string s(otlp_endpoint);
        trim(s);
        if (s.empty()) {
            return "";
        }
        ...
        if (scheme_pos != std::string::npos) {
            scheme = s.substr(0, scheme_pos);
            host_start = scheme_pos + 3;
        }
        size_t slash_pos = s.find('/', host_start);
        std::string hostport = slash_pos == std::string::npos ? s.substr(host_start) : s.substr(host_start, slash_pos - host_start);
        trim(hostport);
        if (hostport.empty()) {
            return "";
        }
        // Replace port to 9411.
        ...
        std::string out = (scheme.empty() ? "http" : scheme) + "://" + host + ":9411/api/v2/spans";
        return out;
    }

    std::string beacon_zipkin_endpoint() {
        // Explicit override for Zipkin endpoint.
        auto raw = beacon_env_str("BEACON_OTEL_ZIPKIN_ENDPOINT");
        if (!raw.empty()) {
            return beacon_normalize_zipkin_endpoint(raw);
        }
        raw = beacon_env_str("OTEL_EXPORTER_ZIPKIN_ENDPOINT");
        if (!raw.empty()) {
            return beacon_normalize_zipkin_endpoint(raw);
        }

        // Convenience: derive from OTLP endpoint if present (collector often runs zipkin receiver at 9411).
        raw = beacon_env_str("BEACON_OTEL_OTLP_ENDPOINT");
        if (!raw.empty()) {
            return beacon_derive_zipkin_from_otlp(raw);
        }
        raw = beacon_env_str("OTEL_EXPORTER_OTLP_ENDPOINT");
        if (!raw.empty()) {
            return beacon_derive_zipkin_from_otlp(raw);
        }
        return "";
    ...
    std::shared_ptr<BeaconZipkinSpan> beacon_start_zipkin_span(const Parser &parser) {
        if (!beacon_env_on("BEACON_OTEL_ENABLED")) {
            return nullptr;
        }
        auto endpoint = beacon_zipkin_endpoint();
        if (endpoint.empty()) {
            return nullptr;
        }
        ...
        const std::string tp = parser["traceparent"];
        const bool has_parent = beacon_parse_traceparent(tp, parent_trace_id, parent_span_id, parent_sampled);

        bool sampled = false;
        if (has_parent) {
            sampled = parent_sampled;
        } else {
            const double ratio_raw = beacon_env_double("BEACON_OTEL_SAMPLE_RATIO", 0.1);
            double ratio = ratio_raw;
            if (ratio < 0.0) {
                ratio = 0.0;
            }
            if (ratio > 1.0) {
            ...
            if (ratio >= 1.0) {
                ...
                // Best-effort probabilistic sampling.
                const std::string r = makeRandStr(1, false);
                const uint8_t v = r.empty() ? 0 : (uint8_t)r[0];
                sampled = ((double)v / 255.0) < ratio;
            }
        }
        if (!sampled) {
            return nullptr;
        ...
        auto span = std::make_shared<BeaconZipkinSpan>();
        span->endpoint = endpoint;
        span->service_name = beacon_env_str("BEACON_OTEL_SERVICE_NAME");
        if (span->service_name.empty()) {
            span->service_name = beacon_env_str("OTEL_SERVICE_NAME");
        }
        if (span->service_name.empty()) {
            span->service_name = "beacon-mediaserver";
        }
        ...
        if (has_parent) {
            ...
            span->trace_id = beacon_random_hex(16);
        }
        span->span_id = beacon_random_hex(8);
        return span;
    }

    void beacon_export_zipkin_span(const std::shared_ptr<BeaconZipkinSpan> &span, int http_status) {
        if (!span || span->endpoint.empty()) {
            return;
        }
        const uint64_t end_us = beacon_now_us();
        uint64_t duration_us = 0;
        if (end_us >= span->start_us) {
            duration_us = end_us - span->start_us;
        }

        Json::Value one(Json::objectValue);
        one["traceId"] = span->trace_id;
        one["id"] = span->span_id;
        if (!span->parent_id.empty()) {
            one["parentId"] = span->parent_id;
        }
        one["name"] = span->name;
        one["kind"] = "SERVER";
        one["timestamp"] = (Json::UInt64)span->start_us;
        one["duration"] = (Json::UInt64)duration_us;
        ...
        if (!span->http_method.empty()) {
            tags["http.method"] = span->http_method;
        }
        if (!span->http_target.empty()) {
            tags["http.target"] = span->http_target;
        }
        tags["http.status_code"] = std::to_string(http_status);
        tags["beacon.component"] = "mediaserver";
        one["tags"] = tags;

        Json::Value arr(Json::arrayValue);
        arr.append(one);
        const std::string body = arr.toStyledString();

        auto requester = std::make_shared<HttpRequester>();
        requester->setMethod("POST");
        requester->setBody(body);
        requester->addHeader("Content-Type", "application/json");
        // Zipkin endpoint usually tolerates no auth; keep best-effort.
        requester->startRequester(span->endpoint, [requester](const SockException &, const Parser &) mutable {
            // Keep best-effort: do not log on failures to avoid noise.
            requester.reset();
        }, 2.0f);
    }
} // namespace
...
static inline void addHttpListener(){
    ...
    NoticeCenter::Instance().addListener(&web_api_tag, Broadcast::kBroadcastHttpRequest, [](BroadcastHttpRequestArgs) {
        ...
        // Beacon observability: best-effort tracing for HTTP API requests.
        // This creates a SERVER span (Zipkin v2 JSON) and exports it asynchronously.
        auto beacon_span = beacon_start_zipkin_span(parser);
        if (beacon_span) {
            auto base_invoker = invoker;
            auto traced_invoker = [base_invoker, beacon_span](int code, const HttpSession::KeyValue &headerOut, const HttpBody::Ptr &body) mutable {
                base_invoker(code, headerOut, body);
                beacon_export_zipkin_span(beacon_span, code);
            };
            ((HttpSession::HttpResponseInvoker &) invoker) = traced_invoker;
        }
