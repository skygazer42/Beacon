...
def run_cmd(cmd, assert_success=False, capture_output=False, env=None) -> bool:
    """
    run cmd
    :param cmd:
    ...
    if not env:
        env = os.environ.copy()
    result = subprocess.run(cmd, shell=False, env=env, capture_output=capture_output)
    # Assert the command ran successfully
    if assert_success and result.returncode != 0:
        print("Command '" + " ".join(cmd) + "' failed with exit status code '" + str(
