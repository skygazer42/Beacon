...
GB28181Process::~GB28181Process() {
    // ========== 修复：GB28181 BYE信令资源释放 ==========
    // 当客户端发送BYE信令断开时，需要主动清理资源
    // Actively clean up resources when client sends BYE to disconnect
    ...
    // 2. 清理RTP解码器
    for (auto &pair : _rtp_decoder) {
        pair.second.reset();
    }
    _rtp_decoder.clear();

    // 3. 清理RTP接收器
    for (auto &pair : _rtp_receiver) {
        pair.second.reset();
    }
    _rtp_receiver.clear();

    // 4. 关闭文件句柄（shared_ptr会自动处理，但显式reset更安全）
    if (_save_file_ps) {
        fflush(_save_file_ps.get());
        _save_file_ps.reset();
    }

    InfoL << "GB28181Process destroyed for stream: " << _media_info.stream;
    // ====================================================
}
