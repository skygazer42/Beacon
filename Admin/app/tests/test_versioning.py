...
class ProjectVersionTest(SimpleTestCase):
    def test_file_version_is_used_without_git_metadata(self):
        with tempfile.TemporaryDirectory() as tmp, mock.patch.object(versioning, "_latest_git_tag", return_value=""):
            root = Path(tmp)
